document.getElementById('userForm').addEventListener('submit', function(event) {
  ...
  function deleteUser(index) {
    var allUsers = JSON.parse(localStorage.getItem('users')) || [];
    allUsers.splice(index, 1); // Remove user at the specified index
    localStorage.setItem('users', JSON.stringify(allUsers)); // Update local storage
    displayAllUserDetails(); // Update the display
  }
